...
import base64
import calendar
from datetime import date, datetime# -*- coding: utf-8 -*-
import os
import io
import math
import time
...
# 🔍 حجم المشهد حول العداد (صغّرناه من 2500م إلى 800م عشان يكون أقرب على الموقع)
SCENE_SIZE_M = 800        # تقدر تغيّره مثلاً 500 أو 1000 حسب ما يعجبك
IMG_SIZE_PX  = 512        # حجم الصورة الناتجة
...
# ==========================
# دوال CDSE (نفس منطق نظام المزارع)
# ==========================
...
def get_cdse_token():
    """
    نخزن التوكن في session_state ونجدده عند الحاجة (مثل نظام المزارع).
    """
...
def download_image(lat: float, lon: float, meter_id: str,
    ...
    وحفظه داخل مجلد العداد مع *علامة حمراء* عند موقع العداد.
    """
    ensure_output_dir()
    meter_folder = os.path.join(OUTPUT_IMG_DIR, str(meter_id))
    os.makedirs(meter_folder, exist_ok=True)
...
"""
    ...
    if r.status_code == 200:
        # 🔴 نرسم علامة + حمراء في مركز الصورة (موقع العداد)
...
def save_results_html(results_rows, gallery, start_date: date, end_date: date) -> bytes:
    """
    ينشئ تقرير HTML:
      - بطاقة لكل عداد (الحالة، درجة التغيّر، المكتب، ...)
      - أفضل صورة + جميع الصور مع التاريخ.
    """
...
def main():
    ...
    for i, (_, row) in enumerate(meters_df.iterrows(), start=1):
        try:
            ...
            # 5) نعرض بطاقة الحالة فورًا بحجم صور صغير
            ...
            with cols[col_idx % 3]:
                ...
                if main_img_path and os.path.exists(main_img_path):
                    st.image(main_img_path, caption="أحدث مشهد (علامة حمراء = موقع العداد)", width=220)
...
IMG_SIZE_PX  = 512        # حجم الصورة الناتجة (صغير عشان الواجهة)
...
def save_results_html(results_rows, gallery, start_date: date, end_date: date) -> bytes:
    ...
    for row in results_rows:
        ...
        # أفضل صورة = آخر صورة قمر صناعي إن وجدت، وإلا المنحنى
        best_img_tag = ""
        if imgs:
            imgs_sorted = sorted(imgs, key=lambda x: x["date"])
            best_info = None
    ...
    for i, (_, row) in enumerate(meters_df.iterrows(), start=1):
        try:
            ...
            with cols[col_idx % 3]:
                ...
                if main_img_path and os.path.exists(main_img_path):
                    st.image(main_img_path, caption="أحدث مشهد", width=220)
